...
type Repo struct {
	...
	// WorkingCopyRoot is an optional absolute filesystem path that callers
	// (typically an LLM agent using the MCP server) can prepend to repo-
	// relative paths from search results in order to Read the file locally.
	// Hound itself does not read working copies — the field is purely
	// informational and is surfaced verbatim by /api/v1/repos and by the
	// MCP wrapper. Leave empty if there is no local working copy mapped to
	// this repo (e.g., the repo is indexed from a remote git URL only).
	WorkingCopyRoot string `json:"working-copy-root,omitempty"`
}
